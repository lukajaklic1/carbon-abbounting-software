...
const T = {
  EN: {
    ...
    subtitle: 'Get started with Carboniqdesk',
    email: 'Email',
    password: 'Password',
    ...
    <div className="min-h-screen bg-[#f7f8fa] flex flex-col items-center justify-center p-4">
      ...
      <div className="w-full max-w-[360px] bg-white rounded-2xl border border-[#ececec] px-8 py-8">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <label className="flex items-start gap-3 cursor-pointer group">
            <div className="relative mt-0.5 shrink-0">
              ...
              <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${
                agreed ? 'bg-[#0f0f10] border-[#0f0f10]' : 'border-[#ececec] group-hover:border-blue-400'
              }`}>
                {agreed && (
            ...
            <span className="text-sm text-[#767676] leading-snug">
              {t.terms1}{' '}
              <a href="#" className="text-[#0f0f10] font-semibold hover:underline">{t.termsLink}</a>
              {' '}{t.terms2}{' '}
              <a href="#" className="text-[#0f0f10] font-semibold hover:underline">{t.privacyLink}</a>
            </span>
          </label>
          ...
          <button type="submit" disabled={!canSubmit}
            className="w-full bg-[#0f0f10] hover:bg-[#2a2a2b] disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-semibold px-4 py-3 rounded-xl transition-colors">
            {loading ? (
      ...
      <div className="mt-5 flex items-center justify-between w-full max-w-[360px]">
        <p className="text-sm text-[#767676]">
          {t.hasAccount}{' '}
          <Link href="/login" className="text-[#0f0f10] hover:underline font-semibold">{t.signIn}</Link>
        </p>
