  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 font-sans antialiased">
      ...
      <div className="max-w-3xl mx-auto px-6 py-10">
        <H1>{sl ? 'SPLOŠNI POGOJI UPORABE STORITVE CARBONIQDESK' : 'CARBONIQDESK TERMS OF SERVICE'}</H1>
        <div className="bg-gray-50 rounded-lg p-4 mb-6 text-sm text-gray-600 space-y-1">
          ...
          <div><strong>{sl ? 'Storitev:' : 'Service:'}</strong> Carboniqdesk</div>
        </div>
        ...
        <P>{sl ? '(1) Ti splošni pogoji uporabe (v nadaljevanju: »Pogoji«) urejajo dostop do in uporabo spletne programske rešitve Carboniqdesk, vključno z njenimi spletnimi aplikacijami, uporabniškimi vmesniki, funkcionalnostmi, izračuni emisij, poročili, dokumentacijo, izvozi, integracijami, programskimi vmesniki ter drugimi povezanimi storitvami, ki jih Ponudnik omogoči uporabnikom (skupaj: »Storitev« ali »Carboniqdesk«).' : '(1) These Terms of Service ("Terms") govern access to and use of the Carboniqdesk web-based software solution, including its web applications, user interfaces, features, emissions calculations, reports, documentation, exports, integrations, APIs and other related services made available by the Provider (together, the "Service" or "Carboniqdesk").'}</P>
        ...
        <P>{sl ? '(6) Storitev je namenjena izključno uporabi med poslovnimi subjekti oziroma za poslovne, poklicne, okoljske, ESG-poročevalske ali komercialne namene (»B2B«). Fizična oseba se lahko tehnično registrira ali uporablja Storitev, vendar z uporabo izjavlja, da jo uporablja v okviru poslovne ali poklicne dejavnosti in ne kot potrošnik za zasebne ali gospodinjske namene.' : '(6) The Service is intended exclusively for business-to-business ("B2B") use in connection with a business, professional, environmental, ESG-reporting or commercial activity. A natural person may technically register or use the Service, but by doing so represents that the Service is being used for business or professional purposes and not as a consumer for private or household purposes.'}</P>
        ...
        <P>{sl ? '(2) »Naročnik« pomeni poslovni subjekt ali drugega poslovnega uporabnika, za katerega se ustvari ali uporablja račun Carboniqdesk.' : '(2) "Customer" means a business entity or other business user for whom a Carboniqdesk account is created or used.'}</P>
        ...
        <P>{sl ? '(4) »Račun« pomeni uporabniški oziroma organizacijski račun Carboniqdesk.' : '(4) "Account" means a user or organisation account in Carboniqdesk.'}</P>
        ...
        <P>{sl ? '(6) »Emisijski podatki« pomenijo zlasti podatke o virih emisij, lokacijah, vozilih, napravah, porabi energije, gorivih, aktivnostnih podatkih, emisijskih faktorjih, obsegu 1, 2 in 3 emisij ter kazalnikih ESG poročanja.' : '(6) "Emissions Data" includes in particular emission source data, locations, vehicles, equipment, energy consumption, fuels, activity data, emission factors, Scope 1, 2 and 3 emissions and ESG reporting indicators.'}</P>
        <P>{sl ? '(7) »Poročevalska dokumentacija« pomeni PDF poročila, CSV uvozne datoteke, Excel preglednice, specifikacije in druge datoteke, ki jih Naročnik naloži ali izvozi iz Storitev.' : '(7) "Reporting Documentation" means PDF reports, CSV import files, Excel spreadsheets, specifications and other files uploaded to or exported from the Service by the Customer.'}</P>
        <P>{sl ? '(8) »Emisijsko poročilo« pomeni izračun ali oceno emisij toplogrednih plinov, pripravljeno v skladu z GHG Protokolom ali drugimi standardi.' : '(8) "Emissions Report" means a calculation or assessment of greenhouse gas emissions prepared in accordance with the GHG Protocol or other applicable standards.'}</P>
        <P>{sl ? '(9) »Rezultat« pomeni vsak izračun, prikaz, poročilo, predlog, oceno ali drug izhod, ustvarjen v Storitvi.' : '(9) "Output" means any calculation, display, report, proposal, estimate or other output created by the Service.'}</P>
        <P>{sl ? '(10) »Beta Storitev« pomeni funkcionalnost ali celotno Storitev, ki je še v testiranju, razvoju ali zgodnji uporabi ter za katero Ponudnik ne zagotavlja enake stopnje stabilnosti, razpoložljivosti ali podpore kot pri morebitni kasnejši komercialni različici.' : '(10) "Beta Service" means a feature or the Service as a whole that is still under testing, development or early use and for which the Provider does not guarantee the same level of stability, availability or support as a later commercial release.'}</P>
        <P>{sl ? '(11) »Podobdelovalec« pomeni tretjo osebo, ki za Ponudnika obdeluje osebne podatke, kadar Ponudnik nastopa kot obdelovalec po GDPR.' : '(11) "Subprocessor" means a third party processing personal data for the Provider where the Provider acts as a processor under the GDPR.'}</P>
        ...
        <P>{sl ? '(4) Naročnik lahko rezultate, poročila, izvozne datoteke in druge dokumente, ki jih ustvari s Storitvijo, uporablja za svoje poslovanje ter jih posreduje svojim partnerjem, revizorjem, regulatorjem in poslovnim partnerjem.' : '(4) The Customer may use and share reports, exports and other outputs created with the Service for its normal business operations and ESG reporting purposes.'}</P>
        ...
        <P>{sl ? '(1) Carboniqdesk, njegova programska koda, arhitektura, podatkovni modeli, uporabniški vmesnik, grafični elementi, dokumentacija, metodologije, funkcionalnosti in drugi elementi, ki niso Uporabniški podatki, so last Ponudnika oziroma njegovih licencodajalcev.' : '(1) Carboniqdesk, its software code, architecture, data models, user interface, graphical elements, documentation, methodologies and features, excluding Customer Data, are owned by the Provider or its licensors.'}</P>
        ...
        <P>{sl ? '(2) Ponudnik ne pridobi lastništva nad emisijskimi podatki, poročili, inventurami emisij, aktivnostnimi podatki, viri emisij ali drugimi poslovnimi podatki Naročnika.' : '(2) The Provider does not acquire ownership of emissions data, reports, emissions inventories, activity data, emission sources or other business data of the Customer.'}</P>
        ...
        <P>{sl ? '(5) Carboniqdesk sam ne uporablja Uporabniških podatkov za treniranje modelov umetne inteligence in Uporabniških podatkov namensko ne pošilja ponudnikom AI zaradi treniranja modelov.' : '(5) Carboniqdesk itself does not use Customer Data to train artificial-intelligence models and does not intentionally send Customer Data to AI providers for model training.'}</P>
        <P>{sl ? '(6) Ponudnik ne uporablja Supabase AI orodij ali Vercel AI produktov za vsebino Uporabniških podatkov kot del običajnega delovanja Carboniqdesk. Supabase v svojih trenutnih Pogojih izrecno določa, da Customer Data, AI Input ali AI Output ne uporablja in tretjim ne dovoli uporabljati za treniranje, fine-tuning ali izboljšavo AI/ML modelov brez predhodnega pisnega soglasja stranke.' : '(6) The Provider does not use Supabase AI tools or Vercel AI products on Customer Data as part of Carboniqdesk\'s ordinary operation. Supabase\'s current Terms expressly state that it will not use, or permit third parties to use, Customer Data, AI Input or AI Output to train, fine-tune or otherwise improve AI or machine-learning models without the customer\'s prior written consent.'}</P>
        <P>{sl ? '(7) Vercelova lastna Politika zasebnosti za določene Hobby/Pro kontekste dopušča uporabo nekaterih kategorij informacij za razvoj in treniranje AI, ob upoštevanju nastavitev oziroma možnosti opt-out, medtem ko se za podatke, ki jih Vercel obdeluje kot pogodbeni obdelovalec, uporablja njegov DPA. Carboniqdesk ne namerava uporabniških emisijskih podatkov ali poročil posredovati Vercelovim AI produktom. Ker Ponudnik ne nadzoruje prihodnjih sprememb pravnih pogojev Vercel ali Supabase, se uporabljajo njihove vsakokrat veljavne politike in DPA v obsegu, v katerem so relevantni.' : '(7) Vercel\'s own Privacy Notice states that in certain Hobby/Pro contexts some categories of information may be used for AI product development and training subject to account preferences and opt-out controls, while data Vercel processes as a contractual processor is governed by its DPA. Carboniqdesk does not intend to submit end-user emissions data or reports to Vercel AI products. Because the Provider does not control future changes to Vercel or Supabase legal terms, their then-current policies and DPAs apply where relevant.'}</P>
        ...
        <P>{sl ? '(2) Za zaupne informacije Naročnika se štejejo zlasti Emisijski podatki, Poročevalska dokumentacija, podatki o virih emisij, energetska poraba, okoljski cilji, ESG strategije, komercialni pogoji in poslovni načrti.' : '(2) Customer Confidential Information includes in particular Emissions Data, Reporting Documentation, emission source data, energy consumption data, environmental targets, ESG strategies, commercial terms and business plans.'}</P>
        ...
        <P>{sl ? '(6) Ponudnik v okviru standardne ali brezplačne beta uporabe ni dolžan podpisati ločenega NDA, individualne pogodbe o zaupnosti ali naročnikovega obrazca. Naročnik lahko po lastni presoji naloži dokumentacijo, za katero ima ustrezna dovoljenja, pri čemer se za zaupnost uporabljajo ta določila. Če Naročnik za posamezne podatke potrebuje posebno pogodbeno raven zaupnosti ali individualen NDA, takih podatkov ne sme naložiti, dokler s Ponudnikom ni sklenjen ločen pisni dogovor.' : '(6) As part of the standard or free beta offering, the Provider is not required to sign a separate NDA, bespoke confidentiality agreement or Customer form. The Customer may upload documentation for which it has sufficient rights, subject to the confidentiality provisions in these Terms. If the Customer requires a specific contractual confidentiality standard or individual NDA for particular information, it must not upload that information unless a separate written agreement has first been entered into with the Provider.'}</P>

        <H1>{sl ? 'IV. EMISIJSKI PODATKI IN ODGOVORNOST NAROČNIKA' : 'IV. EMISSIONS DATA AND CUSTOMER RESPONSIBILITY'}</H1>

        <H2>{sl ? '9. člen – Narava izračunov in rezultatov' : 'Article 9 – Nature of calculations and outputs'}</H2>
        <P>{sl ? '(1) Carboniqdesk je programsko orodje za pomoč pri merjenju, upravljanju in poročanju o emisijah toplogrednih plinov v skladu z GHG Protokolom in drugimi uveljavljenimi standardi.' : '(1) Carboniqdesk is a software tool assisting with measurement, management and reporting of greenhouse gas emissions in accordance with the GHG Protocol and other recognised standards.'}</P>
        <P>{sl ? '(2) Rezultati so odvisni od vhodnih podatkov, nastavitev, emisijskih faktorjev, predpostavk, konfiguracije in načina uporabe Storitev.' : '(2) Outputs depend on inputs, settings, emission factors, assumptions, configuration and the manner in which the Service is used.'}</P>
        <P>{sl ? '(3) Ponudnik ne jamči, da posamezno Emisijsko poročilo ali izračun predstavlja dejanske absolutne emisije podjetja, izpolnjuje zahteve vsakokratnega regulatorja ali je primerno za neposredno predložitev zunanjim revizorjem brez lastnega preverjanja.' : '(3) The Provider does not warrant that an Emissions Report or calculation represents the actual absolute emissions of the Customer\'s organisation, fulfils the requirements of any particular regulator or is suitable for direct submission to external auditors without independent verification.'}</P>
        <P>{sl ? '(4) Rezultati ne predstavljajo pravnega, računovodskega, davčnega, okoljskega ali drugega reguliranega svetovanja.' : '(4) Outputs do not constitute legal, accounting, tax, environmental or other regulated professional advice.'}</P>
        <P>{sl ? '(5) Avtomatski izračun emisij ne nadomešča strokovne presoje okoljskega svetovalca, ESG strokovnjaka, pooblaščenega revizorja ali druge ustrezno usposobljene osebe.' : '(5) Automated emissions calculations do not replace the professional judgment of an environmental consultant, ESG specialist, certified auditor or other appropriately qualified person.'}</P>

        <H2>{sl ? '10. člen – Preverjanje pred uporabo' : 'Article 10 – Verification before use'}</H2>
        <P>{sl ? '(1) Naročnik je odgovoren za pravilnost in popolnost vnesenih podatkov, vključno z aktivnostnimi podatki, emisijskimi faktorji, porabo energije, gorivom, podatki o vozilih, napravah, lokacijah, dobavni verigi in drugimi vhodnimi podatki.' : '(1) The Customer is responsible for the accuracy and completeness of inputs, including activity data, emission factors, energy consumption, fuels, vehicle data, equipment, locations, supply chain data and other input data.'}</P>
        <P>{sl ? '(2) Naročnik mora pred predložitvijo Emisijskega poročila zunanjim strankam, regulatorjem ali revizorjem preveriti vse bistvene izračune in podatke.' : '(2) Before submitting an Emissions Report to external parties, regulators or auditors, the Customer must verify all material calculations and data.'}</P>
        <P>{sl ? '(3) Naročnik sam odloča, ali bo Rezultat uporabil kot podlago za poslovno odločitev, ESG poročanje, regulatorno predložitev ali drugo uporabo.' : '(3) The Customer decides whether and how to rely on an Output for business decisions, ESG reporting, regulatory submission or other use.'}</P>
        ...
        <P>{sl ? '(5) Naročnik je odgovoren za končno vsebino in točnost emisijskih poročil, ki jih predloži zunanjim naslovnikom.' : '(5) The Customer is solely responsible for the final content and accuracy of emissions reports submitted to external recipients.'}</P>
        ...
        <P>{sl ? '(1) Carboniqdesk je v trenutni fazi brezplačna beta različica. Ponudnik namerava tudi po koncu beta obdobja ohraniti vsaj en brezplačen paket, vendar lahko njegov obseg, omejitve, kvote in vključene funkcionalnosti kadarkoli spremeni v skladu s temi Pogoji.' : '(1) Carboniqdesk is currently provided as a free beta version. The Provider currently intends to retain at least one free plan after the beta period, but may change its scope, limits, quotas and included features in accordance with these Terms.'}</P>
        ...
        <P>{sl ? '(1) Carboniqdesk uporablja zunanje ponudnike infrastrukture in programske platforme.' : '(1) Carboniqdesk relies on external infrastructure and software-platform providers.'}</P>
        ...
        <P>{sl ? '(4) Naročnik v Carboniqdesk shranjuje emisijske podatke, aktivnostne podatke, poročila in druge podporne datoteke. Če Uporabnik datoteke ne izbriše in Račun ostane aktiven, je namen Storitve, da se take datoteke hranijo kot del Uporabniških podatkov, dokler so potrebne za uporabo Storitev. To ne pomeni jamstva trajne ali neskončne hrambe in ne nadomešča Naročnikove lastne arhivske kopije.' : '(4) The Customer stores emissions data, activity data, reports and other supporting files in Carboniqdesk. If a User does not delete a file and the Account remains active, the Service is intended to retain such files as Customer Data for as long as they are needed for use of the Service. This is not a guarantee of permanent or indefinite retention and does not replace the Customer\'s own archival copy.'}</P>
        <P>{sl ? '(5) Supabase vsak projekt namesti v eno primarno regijo. Supabase trenutno omogoča splošno evropsko regijo Central EU (Frankfurt) in več specifičnih evropskih AWS regij, med drugim Ireland, London, Paris, Frankfurt, Zurich in Stockholm. Natančna primarna regija konkretnega Carboniqdesk Supabase projekta je tehnična nastavitev projekta in je ni mogoče zanesljivo ugotoviti samo iz javnih pogojev Supabase; zato Carboniqdesk ne daje izjave, da se vsi podatki hranijo izključno v Sloveniji ali izključno v EGP, dokler ta nastavitev ni posebej potrjena.' : '(5) Supabase deploys each project to one primary region. Supabase currently offers a general Central EU (Frankfurt) region and several specific European AWS regions, including Ireland, London, Paris, Frankfurt, Zurich and Stockholm. The exact primary region of the specific Carboniqdesk Supabase project is a project-level technical setting and cannot reliably be determined from Supabase\'s public legal terms alone; Carboniqdesk therefore does not represent that all data is stored exclusively in Slovenia or exclusively in the EEA unless and until that configuration is separately verified.'}</P>
        ...
        <P>{sl ? '(2) Supabase priporoča uporabnikom brezplačnega načrta, da sami redno izvažajo podatke in vzdržujejo zunanje varnostne kopije. Carboniqdesk trenutno ne zagotavlja avtomatiziranega celovitega izvoza vseh podatkov, zato je Naročnik odgovoren, da poslovno kritične emisijske podatke in poročevalsko dokumentacijo po potrebi hrani tudi v lastnih sistemih.' : '(2) Supabase recommends that Free-tier users regularly export their own data and maintain off-site backups. Carboniqdesk does not currently provide a complete automated self-service export of all data, so the Customer remains responsible for retaining business-critical emissions data and reporting documentation in its own systems where appropriate.'}</P>
        ...
        <P>{sl ? '(1) Delovanje Carboniqdesk je lahko odvisno od Vercel, Supabase, ponudnikov DNS, e-pošte, internetnih omrežij in drugih tretjih storitev.' : '(1) Carboniqdesk may depend on Vercel, Supabase, DNS providers, email providers, internet networks and other third-party services.'}</P>
        ...
        <P>{sl ? '(3) Kadar izpad ali napaka tretje storitve bistveno vpliva na Carboniqdesk, si bo Ponudnik prizadeval stanje odpraviti ali najti razumno alternativno rešitev, če je to tehnično in ekonomsko izvedljivo.' : '(3) Where a third-party outage materially affects Carboniqdesk, the Provider will use reasonable efforts to restore functionality or find a reasonable alternative where technically and economically practicable.'}</P>
        ...
        <P>{sl ? '(1) Carboniqdesk je v času objave teh Pogojev na voljo brezplačno kot beta različica, razen če je posameznemu Naročniku pisno sporočeno drugače.' : '(1) At the date of these Terms, Carboniqdesk is available free of charge as a beta unless otherwise expressly communicated to a particular Customer.'}</P>
        ...
        <P>{sl ? '(4) Carboniqdesk trenutno ne zagotavlja celovitega self-service izvoza vseh podatkov. Pred zaprtjem Računa lahko Naročnik kontaktira info@bimetric.si glede razpoložljivih možnosti izvoza.' : '(4) Carboniqdesk does not currently provide a complete self-service export of all data. Before Account closure, the Customer may contact info@bimetric.si regarding available export options.'}</P>
        ...
        <P>{sl ? '(3) Določeni tehnični podatki ali kopije lahko začasno obstajajo v infrastrukturi podobdelovalcev, dnevnikih, predpomnilnikih ali sistemih za dostavo storitev. Takšne kopije niso pogodbeno zagotovljena varnostna kopija Carboniqdesk in niso na voljo kot storitev obnove.' : '(3) Certain technical data or copies may temporarily exist within subprocessor infrastructure, logs, caches or service-delivery systems. Such copies are not a contractually guaranteed Carboniqdesk backup and are not provided as a recovery service.'}</P>
        ...
        <P>{sl ? '(1) Naročnik in vsak Uporabnik z registracijo, sprejemom teh Pogojev oziroma uporabo Carboniqdesk izrecno potrjujeta in se strinjata, da je programska oprema po svoji naravi lahko predmet napak, pomanjkljivosti, nepravilnega delovanja, prekinitev in nepričakovanih rezultatov ter da nobene programske opreme ni mogoče razumno zagotavljati kot popolnoma brezhibne v vseh okoliščinah.' : '(1) By registering, accepting these Terms or using Carboniqdesk, the Customer and each User expressly acknowledge and agree that software by its nature may contain defects, deficiencies, malfunctions, interruptions and unexpected results and that no software can reasonably be guaranteed to be completely error-free in all circumstances.'}</P>
        ...
        <P>{sl ? '(3) Naročnik izrecno sprejema, da lahko Carboniqdesk vsebuje programske napake, napake v formulah ali emisijskih faktorjih, nepravilne izračune emisij, napake pri uvozu ali izvozu podatkov, napake pri generiranju poročil ali druge pomanjkljivosti.' : '(3) The Customer expressly accepts that Carboniqdesk may contain software bugs, errors in formulas or emission factors, incorrect emissions calculations, data import or export errors, report-generation errors or other defects.'}</P>
        <P>{sl ? '(4) Ponudnik ne daje jamstva, da so Emisijska poročila, izračuni ali drugi Rezultati popolni, točni ali primerni za neposredno uporabo brez preverjanja.' : '(4) The Provider does not warrant that Emissions Reports, calculations or other Outputs are complete, accurate or suitable for direct use without verification.'}</P>
        <P>{sl ? '(5) Naročnik se strinja, da obstoj napake v Storitvi sam po sebi ne pomeni, da Ponudnik prevzema odgovornost za poslovne posledice uporabe napačnega Rezultata.' : '(5) The Customer agrees that the existence of an error in the Service does not by itself mean that the Provider assumes liability for business consequences resulting from use of an incorrect Output.'}</P>

        <H2>{sl ? '27. člen – Obvezno preverjanje emisijskih poročil in podatkov' : 'Article 27 – Mandatory verification of emissions reports and data'}</H2>
        ...
        <P>{sl ? '(2) Preverjanje iz prejšnjega odstavka vključuje zlasti aktivnostne podatke, emisijske faktorje, porabo energije, podatke o virih emisij, razporeditev na obsege 1, 2 in 3, korekcijske faktorje, bazna leta, merilne enote, komercialne pogoje in končne izračune emisij.' : '(2) Such verification includes activity data, emission factors, energy consumption, emission source data, allocation to Scopes 1, 2 and 3, correction factors, base years, units of measurement, commercial terms and final emissions calculations.'}</P>
        <P>{sl ? '(3) Naročnik se izrecno strinja, da Carboniqdesk predstavlja pripomoček za merjenje in poročanje o emisijah ter ne nadomešča strokovne kontrole okoljskega svetovalca, ESG strokovnjaka, pooblaščenega revizorja ali druge ustrezno usposobljene osebe.' : '(3) The Customer expressly acknowledges and agrees that Carboniqdesk is an assistance tool for emissions measurement and reporting and does not replace professional review by an environmental consultant, ESG specialist, certified auditor or other appropriately qualified person.'}</P>
        <P>{sl ? '(4) Preden Naročnik oziroma Uporabnik Emisijsko poročilo predloži regulatorju, revizorju, investitorju ali drugi tretji osebi, mora sam preveriti pravilnost relevantnih podatkov in Rezultata.' : '(4) Before submitting an Emissions Report to a regulator, auditor, investor or other third party, the Customer or User must independently verify the relevant data and Output.'}</P>
        <P>{sl ? '(5) Naročnik je odgovoren za končno vsebino, točnost in skladnost vsakega poročila, ki ga predloži tretji osebi, ne glede na to, ali je bilo poročilo v celoti ali delno ustvarjeno s Carboniqdesk.' : '(5) The Customer is responsible for the final content, accuracy and compliance of every report delivered to a third party, whether generated wholly or partly using Carboniqdesk.'}</P>
        ...
        <Ul items={sl ? [
          ...
          'napačne konfiguracije, nastavitev, emisijskih faktorjev, meja organizacije, virov emisij ali drugih parametrov;',
          'napačne ali neustrezne uporabe Storitev;',
          'neizvedenega ali nezadostnega preverjanja Rezultatov;',
          'programske napake, napake v izračunu emisij, napake v programski logiki ali nepravilnega delovanja Storitev;',
          'napake pri prikazu, uvozu, izvozu, shranjevanju ali generiranju poročil;',
          'nepravilnih izračunov obsega 1, 2 ali 3 emisij;',
          'napačnih aktivnostnih podatkov, emisijskih faktorjev, merilnih enot ali korekcijskih faktorjev;',
          'odločitve Naročnika ali tretje osebe, sprejete na podlagi Rezultata;',
          'poročila, regulatorne predložitve ali pogodbe, ki jo je Naročnik posredoval na podlagi Rezultata.',
        ] : [
          'incorrect, incomplete, outdated or improperly entered Customer or User data;',
          'incorrect configuration, settings, emission factors, organisational boundaries, emission sources or other parameters;',
          'incorrect or inappropriate use of the Service;',
          'failure to verify or insufficient verification of Outputs;',
          'a software bug, emissions calculation error, logic error or malfunction of the Service;',
          'display, import, export, storage or report-generation errors;',
          'incorrect Scope 1, 2 or 3 emissions calculations;',
          'incorrect activity data, emission factors, units of measurement or correction factors;',
          'a decision made by the Customer or a third party based on an Output;',
          'a report, regulatory submission or contract submitted by the Customer based on an Output.',
        ]} />
        <P>{sl ? '(2) Naročnik izrecno potrjuje in se strinja, da je sam odgovoren za končno strokovno in vsebinsko kontrolo Rezultatov, tudi kadar je napaka v Rezultatu posledica programske napake Carboniqdesk.' : '(2) The Customer expressly acknowledges and agrees that it remains responsible for final professional and substantive review of Outputs even where an incorrect Output results from a Carboniqdesk software error.'}</P>
        ...
        <P>{sl ? '(1) V največjem obsegu, ki ga dovoljuje veljavno pravo, Ponudnik v nobenem primeru ne odgovarja za posredno, posledično, posebno, naključno ali kaznovalno škodo oziroma za izgubljeni dobiček, izgubljeni prihodek, izgubo poslovne priložnosti, izgubne regulatorne postopke, globe ali sankcije, ki izhajajo iz napačnih emisijskih poročil, prekinitev poslovanja, stroške ponovnega poročanja, pogodbene kazni ali druge primerljive poslovne posledice.' : '(1) To the fullest extent permitted by applicable law, the Provider shall in no event be liable for indirect, consequential, special, incidental or punitive damages, or for lost profits, lost revenue, lost business opportunities, regulatory proceedings, fines or sanctions arising from incorrect emissions reports, business interruption, re-reporting costs, contractual penalties or comparable business losses.'}</P>
        ...
        <P>{sl ? '(2) Za osebne podatke, ki jih Naročnik vnese v Carboniqdesk v okviru lastnega poslovanja, na primer podatke o svojih zaposlenih, dobaviteljih ali poslovnih partnerjih v zvezi s poročanjem o emisijah, Naročnik praviloma nastopa kot upravljavec, Ponudnik pa kot obdelovalec po navodilih Naročnika.' : '(2) For personal data entered into Carboniqdesk by the Customer as part of its own business operations, such as information relating to the Customer\'s employees, suppliers or business partners in connection with emissions reporting, the Customer generally acts as controller and the Provider acts as processor on the Customer\'s documented instructions.'}</P>
        ...
        <P>{sl ? '(4) Podrobnejše informacije o obdelavi podatkov, kjer Ponudnik nastopa kot upravljavec, so določene v Politiki zasebnosti Carboniqdesk.' : '(4) Processing for which the Provider acts as controller is described further in the Carboniqdesk Privacy Policy.'}</P>
        ...
        <P>{sl ? '(2) Predmet obdelave je zagotavljanje Storitev Carboniqdesk, vključno z gostovanjem podatkov, shranjevanjem, prikazovanjem, prenosom, avtentikacijo, podporo, varnostjo, izračuni emisij ter drugimi funkcijami, ki jih Naročnik uporablja.' : '(2) The subject matter is the provision of Carboniqdesk, including hosting, storage, display, transmission, authentication, support, security, emissions calculations and related functions selected by the Customer.'}</P>
        ...
        <Ul items={sl ? [
          ...
          'kontaktne osebe pri dobaviteljih in poslovnih partnerjih;',
          'osebe, katerih podatke Naročnik vnese v Storitev v zvezi s poročanjem o emisijah.',
        ] : [
          'employees, contractors and Users of the Customer;',
          'contacts at suppliers and business partners;',
          'persons whose details the Customer submits in connection with emissions reporting.',
        ]} />
        ...
        <P>{sl ? '(1) Ker sta Supabase in Vercel mednarodna ponudnika infrastrukture in uporabljata oziroma lahko uporabljata infrastrukturo ter podobdelovalce zunaj Evropskega gospodarskega prostora, lahko pri uporabi Carboniqdesk pride do mednarodnih prenosov osebnih podatkov.' : '(1) Because Supabase and Vercel are international infrastructure providers and use or may use infrastructure and subprocessors outside the EEA, use of Carboniqdesk may involve international transfers of personal data.'}</P>
        <P>{sl ? '(2) Supabase projekt ima eno primarno regijo, vendar javni pogoji sami ne razkrivajo primarne regije konkretnega Carboniqdesk projekta. Supabase poleg tega navaja možno obdelavo v ZDA in drugih državah, kjer imajo Supabase ali njegovi izvajalci infrastrukturo.' : '(2) A Supabase project has one primary region, but the public legal terms do not identify the primary region of the specific Carboniqdesk project. Supabase additionally states that processing may occur in the United States and other countries where Supabase or its contractors maintain facilities.'}</P>
        ...
        <P>{sl ? '(1) Obdelava osebnih podatkov, pri kateri Ponudnik nastopa kot upravljavec, je podrobneje opisana v ločeni Politiki zasebnosti Carboniqdesk.' : '(1) Processing for which the Provider acts as controller is described in the separate Carboniqdesk Privacy Policy.'}</P>
        ...
        <P>{sl ? '(1) Carboniqdesk lahko uporablja nujno potrebne piškotke ali podobne tehnologije za prijavo, varnost, shranjevanje seje in osnovno delovanje.' : '(1) Carboniqdesk may use strictly necessary cookies or similar technologies for login, security, session storage and core operation.'}</P>
        <P>{sl ? '(2) Carboniqdesk trenutno ne uporablja neobveznega spletnega analytics sistema, session replay sistema ali namenskega error-monitoring ponudnika za sledenje vedenju končnih uporabnikov.' : '(2) Carboniqdesk currently does not use an optional web-analytics system, session-replay system or dedicated error-monitoring provider to track end-user behaviour.'}</P>
        ...
        <P>{sl ? '(1) Ponudnik lahko v prihodnje prenese upravljanje, razvoj ali zagotavljanje Carboniqdesk na novo ustanovljeno ali drugo povezano pravno osebo oziroma poslovni subjekt, če je tak prenos zakonit.' : '(1) The Provider may in future transfer the operation, development or provision of Carboniqdesk to a newly established or other affiliated legal entity or business where lawful.'}</P>
