...
export default function EditVehicleForm({ vehicle }: { vehicle: any }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await updateVehicle(vehicle.id, new FormData(e.currentTarget))
  ...
  async function handleDelete() {
    ...
    if (IS_MOCK) { router.push('/app/vehicles'); return }
    await deleteVehicle(vehicle.id)
    router.push('/app/vehicles')
  }

  return (
    <FormCard title="Edit Vehicle" backHref="/app/vehicles">
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        <FormField label="Vehicle name">
          <FormInput name="name" defaultValue={vehicle.name} required />
        </FormField>
        <FormField label="Vehicle type">
          <FormSelect name="vehicle_type" defaultValue={vehicle.vehicle_type}>
        ...
        <FormField label="Fuel type">
          <FormSelect name="fuel_type" defaultValue={vehicle.fuel_type}>
        ...
        <FormField label="Size category">
          <FormSelect name="size_category" defaultValue={vehicle.size_category ?? 'average'}>
