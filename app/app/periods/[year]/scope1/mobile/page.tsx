...
const EMPTY_FORM = { fuel_type: 'diesel', quantity: '', unit: 'L', data_source: '', notes: '' }
...
export default function Scope1MobilePage() {
  ...
  function openAdd(vehicle: any) {
    ...
    setForm({ fuel_type: ft, quantity: '', unit: FUEL_FACTORS[ft]?.unit ?? 'L', data_source: '', notes: '' })
    setActiveVehicle(vehicle); setError(''); setShowModal(true)
  }

  function openEdit(vehicle: any) {
    const e = entriesMap[vehicle.id]
    setForm({ fuel_type: e.fuel_type ?? 'diesel', quantity: Number(e.quantity ?? 0).toLocaleString('sl-SI', { maximumFractionDigits: 6 }), unit: e.unit ?? 'L', data_source: e.data_source ?? '', notes: e.notes ?? '' })
    setActiveVehicle(vehicle); setError(''); setShowModal(true)
  }
  ...
  return (
    <div className="p-4 lg:p-8">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">{t('Poraba goriva – vozila', 'Vehicle fuel consumption')}</h1>
        <p className="text-sm text-gray-400 mt-0.5">{t('Direktne emisije iz mobilnega zgorevanja', 'Direct emissions from mobile combustion')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-xl py-14 text-center">
          ...
          <p className="text-sm text-gray-500 mb-1">{t('Ni aktivnih vozil.', 'No active vehicles.')}</p>
          <p className="text-xs text-gray-400">{t('Najprej dodajte vozila v razdelku Vozila.', 'First add vehicles in the Vehicles section.')}</p>
        </div>
      ) : (
                  ...
                  <tr key={v.id} className={`hover:bg-gray-50 transition-colors ${i !== 0 ? 'border-t border-gray-100' : ''}`}>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-blue-50 rounded-lg flex items-center justify-center shrink-0 text-base leading-none">
                          {VEHICLE_EMOJI[v.vehicle_type] ?? '🚗'}
                        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 z-10 bg-white border-b border-gray-100 px-6 py-5 flex items-center justify-between rounded-t-2xl">
              <div>
                <h2 className="text-lg font-bold text-gray-900">{entriesMap[activeVehicle.id] ? t('Uredi vnos', 'Edit entry') : t('Dodaj porabo', 'Add consumption')}</h2>
                <p className="text-xs text-gray-400 mt-0.5">{activeVehicle.name} · Scope 1 · {year}</p>
              </div>
            ...
            <div className="sticky bottom-0 bg-white border-t border-gray-100 px-6 py-4 flex gap-3 rounded-b-2xl">
              ...
              <button onClick={handleSave} disabled={saving || !form.quantity || parseFloat(String(form.quantity).replace(',', '.')) <= 0}
