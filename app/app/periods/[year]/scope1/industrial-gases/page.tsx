  ...
  return (
    <div className="p-4 lg:p-8">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">{t('Industrijski plini – oprema', 'Industrial gases – equipment')}</h1>
        <p className="text-sm text-gray-400 mt-0.5">{t('Direktne emisije iz izhajanja industrijskih plinov', 'Direct emissions from industrial gas leakage')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-xl py-14 text-center">
          ...
          <p className="text-sm text-gray-500 mb-1">{t('Ni opreme z industrijskimi plini.', 'No industrial gas equipment.')}</p>
          <p className="text-xs text-gray-400">{t('Označite opremo kot "Uporablja industrijske pline" v razdelku Oprema.', 'Mark equipment as "Uses industrial gases" in the Equipment section.')}</p>
        </div>
      ) : (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                ...
                <th className="text-left text-xs font-semibold text-gray-400 uppercase tracking-wider px-5 py-3">{t('Plin', 'Gas')}</th>
                <th className="text-left text-xs font-semibold text-gray-400 uppercase tracking-wider px-5 py-3">{t('Količina', 'Quantity')} (kg)</th>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 z-10 bg-white border-b border-gray-100 px-6 py-5 flex items-center justify-between rounded-t-2xl">
              <div>
                <h2 className="text-lg font-bold text-gray-900">{entriesMap[activeItem.id] ? t('Uredi vnos', 'Edit entry') : t('Dodaj plin', 'Add gas')}</h2>
