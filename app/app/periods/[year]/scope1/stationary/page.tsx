    ...
    <div className="flex flex-col h-full">
      <div className="flex flex-wrap items-center justify-between gap-2 px-4 sm:px-6 border-b border-gray-200 min-h-[57px] py-3 sm:h-[57px] sm:py-0 shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <p className="text-sm text-gray-500 truncate">{t('Vnesite letno porabo zemeljskega plina znotraj poročevalskega obdobja po lokacijah.', 'Enter the annual natural gas consumption by location for the reporting period.')}</p>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[80vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-2 py-3 space-y-1.5">
              {(() => {
                // Active locations + inactive-but-selected locations (so they can be unselected)
                ...
                if (modalList.length === 0) return <div className="mx-4 my-3 border border-gray-200 bg-white rounded-xl flex flex-col items-center justify-center py-10 px-6 text-center"><Building2 className="h-8 w-8 mb-3" style={{ color: '#e4e4e8' }} /><p className="text-sm font-medium text-gray-900">{t('Ni aktivnih lokacij', 'No active locations')}</p><p className="text-sm text-gray-500 mt-1 max-w-[280px] leading-relaxed">{t('Dodajte lokacijo, ki uporablja zemeljski plin.', 'Add a location that uses natural gas.')}</p></div>
