...
import { Wrench, Plus, Pencil, X, Leaf, Check } from 'lucide-react'
  ...
  return (
    <div className="p-4 lg:p-8">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-[#031f18]">{t('Gorivo opreme', 'Equipment fuel')}</h1>
        <p className="text-sm text-[#767676] mt-0.5">{t('Direktne emisije iz zgorevanja goriva v opremi', 'Direct emissions from fuel combustion in equipment')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-[#ececec] rounded-xl py-14 text-center">
          <Wrench className="h-7 w-7 text-gray-300 mx-auto mb-3" />
          <p className="text-sm text-[#767676] mb-1">{t('Ni opreme z gorivom.', 'No fuel-using equipment.')}</p>
          <p className="text-xs text-[#767676]">{t('Označite opremo kot "Uporablja gorivo" v razdelku Oprema.', 'Mark equipment as "Uses fuel" in the Equipment section.')}</p>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 z-10 bg-white border-b border-[#ececec] px-6 py-5 flex items-center justify-between rounded-t-2xl">
              <div>
                <h2 className="text-lg font-bold text-[#031f18]">{entriesMap[activeItem.id] ? t('Uredi vnos', 'Edit entry') : t('Dodaj porabo', 'Add consumption')}</h2>
