...
export default function Scope1EquipmentFuelPage() {
  ...
  async function saveSelection() {
    ...
    if (toAdd.length) await sup.from('period_equipment').insert(toAdd.map(equipment_id => ({ reporting_period_id: period.id, equipment_id, organization_id: orgId, scope_type: 'fuel' })))
  ...
  async function load(silent = false) {
    ...
    try {
      ...
      const [{ data: pd }, { data: equip }, { data: peRows }, { data: ents }] = await Promise.all([
        ...
        supabase.from('equipment').select('id, name, fuel_type').eq('organization_id', org.id).eq('is_active', true).eq('uses_fuel', true).order('name'),
        supabase.from('period_equipment').select('id, equipment_id, reporting_period_id').eq('organization_id', org.id).eq('scope_type', 'fuel'),
        supabase.from('scope1_equipment_fuel').select('*').eq('organization_id', org.id),
      ])
      ...
      if (reportEquIds.length > 0) {
        const { data: reportEqu } = await supabase.from('equipment').select('id, name, fuel_type, is_active').in('id', reportEquIds)
  ...
  async function handleDelete() {
    ...
    const supabase = createClient()
    await supabase.from('scope1_equipment_fuel').delete().eq('id', entry.id)
    ...
    <div className="flex flex-col h-full">
      <div className="flex flex-wrap items-center justify-between gap-2 px-4 sm:px-6 border-b border-gray-200 min-h-[57px] py-3 sm:h-[57px] sm:py-0 shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <p className="text-sm text-gray-500 truncate">{t('Vnesite letno porabo goriva znotraj poročevalskega obdobja po posameznem kosu opreme.', 'Enter the annual fuel consumption for the reporting period per piece of equipment.')}</p>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState iconNode={<IconEngine size={32} />} title={t('Ni opreme z gorivom', 'No fuel-using equipment')} subtitle={t('Dodajte opremo, ki uporablja gorivo.', 'Add equipment that uses fuel.')} />
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{backgroundColor:'#e5eeff',border:'1px solid #d6e5ff'}}>
                          <Wrench className="h-3.5 w-3.5" style={{color:'#215bcf'}} />
                        </div>
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[80vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-2 py-3 space-y-1.5">
              {(() => {
                ...
                if (modalList.length === 0) return <div className="mx-4 my-3 border border-gray-200 bg-white rounded-xl flex flex-col items-center justify-center py-10 px-6 text-center"><Wrench className="h-8 w-8 mb-3" style={{ color: '#e4e4e8' }} /><p className="text-sm font-medium text-gray-900">{t('Ni opreme z gorivom', 'No fuel-using equipment')}</p><p className="text-sm text-gray-500 mt-1 max-w-[280px] leading-relaxed">{t('Dodajte opremo, ki uporablja gorivo.', 'Add equipment that uses fuel.')}</p></div>
                    ...
                    <div key={item.id} className={`rounded-xl border transition-all overflow-hidden ${inactive ? 'opacity-60' : ''}`} style={{ borderColor: checked ? '#215bcf' : '#e5e7eb' }}>
                      <label className="flex items-center gap-3 px-4 py-3 cursor-pointer" style={{ backgroundColor: checked ? '#f0f5ff' : '#fff' }}>
                        ...
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border" style={{ backgroundColor: checked ? '#e5eeff' : '#f3f4f6', borderColor: checked ? '#c7d9ff' : '#e5e7eb' }}>
                          <Wrench className={`w-3.5 h-3.5 ${checked ? 'text-[#215bcf]' : 'text-gray-400'}`} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-gray-900 truncate">{item.name}</p>
                          {item.fuel_type && <p className="text-xs text-gray-500 truncate">{t(FUEL_FACTORS[item.fuel_type]?.label_sl ?? item.fuel_type, FUEL_FACTORS[item.fuel_type]?.label_en ?? item.fuel_type)}</p>}
