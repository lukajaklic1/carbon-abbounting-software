...
const ALL_FUEL_KEYS = Object.keys(FUEL_FACTORS)
  ...
  return (
    <div className="p-4 lg:p-8">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">{t('Gorivo opreme', 'Equipment fuel')}</h1>
        <p className="text-sm text-gray-400 mt-0.5">{t('Direktne emisije iz zgorevanja goriva v opremi', 'Direct emissions from fuel combustion in equipment')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-xl py-14 text-center">
          ...
          <p className="text-sm text-gray-500 mb-1">{t('Ni opreme z gorivom.', 'No fuel-using equipment.')}</p>
          <p className="text-xs text-gray-400">{t('Označite opremo kot "Uporablja gorivo" v razdelku Oprema.', 'Mark equipment as "Uses fuel" in the Equipment section.')}</p>
        </div>
      ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 z-10 bg-white border-b border-gray-100 px-6 py-5 flex items-center justify-between rounded-t-2xl">
              <div>
                <h2 className="text-lg font-bold text-gray-900">{entriesMap[activeItem.id] ? t('Uredi vnos', 'Edit entry') : t('Dodaj porabo', 'Add consumption')}</h2>
