  ...
  return (
    <div className="p-4 lg:p-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-[#767676] mt-0.5">{t('Direktne emisije iz izhajanja hladil', 'Direct emissions from refrigerant leakage')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-[#ececec] rounded-xl py-14 text-center">
          ...
          <p className="text-sm text-[#767676] mb-1">{t('Ni opreme s hladivi.', 'No refrigerant-using equipment.')}</p>
          <p className="text-xs text-[#767676]">{t('Označite opremo kot "Uporablja hladiva" v razdelku Oprema.', 'Mark equipment as "Uses refrigerants" in the Equipment section.')}</p>
        </div>
      ) : (
          ...
          <table className="w-full">
            <thead className="bg-[#fafafc]">
              <tr className="border-b border-[#ececec] bg-[#f9f9f9]/50">
                <th className="text-left text-xs font-semibold text-[#767676] uppercase tracking-wider px-5 py-3">{t('Oprema', 'Equipment')}</th>
                <th className="text-left text-xs font-semibold text-[#767676] uppercase tracking-wider px-5 py-3">{t('Hladivo', 'Refrigerant')}</th>
                <th className="text-left text-xs font-semibold text-[#767676] uppercase tracking-wider px-5 py-3">{t('Dopolnitev', 'Top-up')} (kg)</th>
            ...
            <tbody>
              {paginated.map((item, i) => {
                ...
                return (
                  <tr key={item.id} className={`hover:bg-[#f9f9f9] transition-colors ${i !== 0 ? 'border-t border-[#ececec]' : ''}`}>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 z-10 bg-white border-b border-[#ececec] px-6 py-5 flex items-center justify-between rounded-t-2xl">
              <div>
                ...
                <p className="text-xs text-[#767676] mt-0.5">{activeItem.name} · Scope 1 · {year}</p>
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  <div className="w-10 px-2 py-2 text-sm bg-[#f9f9f9] border border-[#ececec] rounded-lg text-[#767676] flex items-center justify-center shrink-0 font-medium">kg</div>
                </div>
              </div>
              {preview !== null && (
