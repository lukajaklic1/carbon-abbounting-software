...
import { Thermometer, Plus, Pencil, X, Leaf, Check, Settings2 } from 'lucide-react'
...
export default function Scope1RefrigerantsPage() {
  ...
  async function saveSelection() {
    ...
    if (toAdd.length) await sup.from('period_equipment').insert(toAdd.map(equipment_id => ({ reporting_period_id: period.id, equipment_id, organization_id: orgId, scope_type: 'refrigerants' })))
  ...
  async function load(silent = false) {
    ...
    try {
      ...
      const [{ data: pd }, { data: equip }, { data: peRows }, { data: ents }] = await Promise.all([
        ...
        supabase.from('period_equipment').select('id, equipment_id, reporting_period_id').eq('organization_id', org.id).eq('scope_type', 'refrigerants'),
      ...
      const reportEquIds = thisPe.map((r: any) => r.equipment_id)
      if (reportEquIds.length > 0) {
        const { data: reportEqu } = await supabase.from('equipment').select('id, name, refrigerant_type, is_active').in('id', reportEquIds)
        setReportEquipmentAll(reportEqu ?? [])
      } else { setReportEquipmentAll([]) }
    ...
    <div className="flex flex-col h-full">
      <div className="flex flex-wrap items-center justify-between gap-2 px-4 sm:px-6 border-b border-gray-200 min-h-[57px] py-3 sm:h-[57px] sm:py-0 shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <p className="text-sm text-gray-500 truncate">{t('Vnesite letno količino napolnjenih hladiv znotraj poročevalskega obdobja po posameznem kosu opreme.', 'Enter the annual refrigerant top-up quantity for the reporting period per piece of equipment.')}</p>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState iconNode={<IconAirConditioning size={32} />} title={t('Ni opreme s hladivi', 'No refrigerant-using equipment')} subtitle={t('Dodajte opremo, ki uporablja hladivo.', 'Add equipment that uses refrigerants.')} />
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                <th className="text-left text-xs font-medium text-gray-500 px-5 py-3">{t('Hladivo', 'Refrigerant')}</th>
                <th className="text-left text-xs font-medium text-gray-500 px-5 py-3">{t('Dopolnitev', 'Top-up')} (kg)</th>
              ...
              {paginated.map((item, i) => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3.5">
                      {entry
                        ? <span className="text-sm font-semibold text-gray-500">{(entry.co2e_kg / 1000).toFixed(2).replace('.', ',')} tCO₂e</span>
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[80vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-2 py-3 space-y-1.5">
              {(() => {
                ...
                if (modalList.length === 0) return <div className="mx-4 my-3 border border-gray-200 bg-white rounded-xl flex flex-col items-center justify-center py-10 px-6 text-center"><Thermometer className="h-8 w-8 mb-3" style={{ color: '#e4e4e8' }} /><p className="text-sm font-medium text-gray-900">{t('Ni opreme s hladivi', 'No refrigerant-using equipment')}</p><p className="text-sm text-gray-500 mt-1 max-w-[280px] leading-relaxed">{t('Dodajte opremo, ki uporablja hladivo.', 'Add equipment that uses refrigerants.')}</p></div>
                    ...
                    <div key={item.id} className={`rounded-xl border transition-all overflow-hidden ${inactive ? 'opacity-60' : ''}`} style={{ borderColor: checked ? '#215bcf' : '#e5e7eb' }}>
                      <label className="flex items-center gap-3 px-4 py-3 cursor-pointer" style={{ backgroundColor: checked ? '#f0f5ff' : '#fff' }}>
                        ...
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border" style={{ backgroundColor: checked ? '#e5eeff' : '#f3f4f6', borderColor: checked ? '#c7d9ff' : '#e5e7eb' }}>
                          <Thermometer className={`w-3.5 h-3.5 ${checked ? 'text-[#215bcf]' : 'text-gray-400'}`} />
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-gray-900 truncate">{item.name}</p>
                          {item.refrigerant_type && <p className="text-xs text-gray-500 truncate">{item.refrigerant_type}</p>}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">{t('Vrsta hladiva', 'Refrigerant type')}</label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">{t('Letna dopolnitev', 'Annual top-up')} (kg) <span className="text-red-400">*</span></label>
                <div className="flex gap-2">
            ...
            <div className="sticky bottom-0 bg-white border-t border-gray-200 px-6 py-4 flex gap-3 rounded-b-2xl">
              ...
              <button onClick={handleSave} disabled={saving || !form.quantity_kg || parseQty(form.quantity_kg) < 0}
                ...
                {entriesMap[activeItem.id] ? t('Shrani', 'Save') : t('Dodaj vnos', 'Add entry')}
              </button>
            </div>
          </div>
        </div>
