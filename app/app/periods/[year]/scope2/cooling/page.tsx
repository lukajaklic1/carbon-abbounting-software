...
export default function Scope2CoolingPage() {
  ...
  async function saveSelection() {
    ...
    if (toAdd.length) await sup.from('period_locations').insert(toAdd.map(location_id => ({ reporting_period_id: period.id, location_id, organization_id: orgId, scope_type: 'cooling' })))
  ...
  async function load(silent = false) {
    ...
    try {
      ...
      const [{ data: pd }, { data: locs }, { data: plRows }, { data: ents }] = await Promise.all([
        ...
        supabase.from('period_locations').select('id, location_id, reporting_period_id').eq('organization_id', org.id).eq('scope_type', 'cooling'),
