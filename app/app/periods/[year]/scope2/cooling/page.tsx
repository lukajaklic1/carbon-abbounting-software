  ...
  return (
    <div className="p-4 lg:p-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-[#767676] mt-0.5">{t('Posredne emisije iz nakupljenega hlajenja', 'Indirect emissions from purchased cooling')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-[#ececec] rounded-xl py-14 text-center">
          ...
          <p className="text-sm text-[#767676] mb-1">{t('Ni lokacij z nakupljenim hlajenjem.', 'No locations with purchased cooling.')}</p>
          <p className="text-xs text-[#767676]">{t('Označite lokacijo kot "uporablja daljinsko hlajenje" v razdelku Lokacije.', 'Mark a location as "uses cooling" in the Locations section.')}</p>
        </div>
      ) : (
          ...
          <table className="w-full">
            <thead className="bg-[#fafafc]">
              <tr className="border-b border-[#ececec] bg-[#f9f9f9]/50">
                ...
                <th className="text-left text-xs font-semibold text-[#767676] uppercase tracking-wider px-5 py-3">{t('Metoda', 'Method')}</th>
              ...
              {paginated.map((loc, i) => {
                ...
                return (
                  <tr key={loc.id} className={`hover:bg-[#f9f9f9] transition-colors ${i !== 0 ? 'border-t border-[#ececec]' : ''}`}>
                    ...
                    <td className="px-5 py-4 text-sm text-[#767676]">{entry ? t(COOLING_FACTORS[entry.method]?.label_sl ?? entry.method, COOLING_FACTORS[entry.method]?.label_en ?? entry.method) : <span className="text-gray-300">—</span>}</td>
