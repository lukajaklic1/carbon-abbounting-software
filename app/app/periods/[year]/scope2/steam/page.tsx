...
export default function Scope2SteamPage() {
  ...
  async function saveSelection() {
    ...
    if (toAdd.length) await sup.from('period_locations').insert(toAdd.map(location_id => ({ reporting_period_id: period.id, location_id, organization_id: orgId, scope_type: 'steam' })))
  ...
  async function load(silent = false) {
    ...
    try {
      ...
      const [{ data: pd }, { data: locs }, { data: plRows }, { data: ents }] = await Promise.all([
        ...
        supabase.from('period_locations').select('id, location_id, reporting_period_id').eq('organization_id', org.id).eq('scope_type', 'steam'),
    ...
    <div className="flex flex-col h-full">
      <div className="flex flex-wrap items-center justify-between gap-2 px-4 sm:px-6 border-b border-gray-200 min-h-[57px] py-3 sm:h-[57px] sm:py-0 shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          <h1 className="text-base font-semibold text-gray-900 shrink-0">{t('Para', 'Steam')}</h1>
          <p className="text-sm text-gray-500 truncate">{t('Vnesite letno porabo pare znotraj poročevalskega obdobja po lokacijah.', 'Enter the annual steam consumption for the reporting period by location.')}</p>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState icon={Wind} title={t('Ni lokacij s kupljeno paro', 'No locations with purchased steam')} subtitle={t('Dodajte lokacijo, ki kupuje paro.', 'Add a location that purchases steam.')} />
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[80vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-2 py-3 space-y-1.5">
              {(() => {
                ...
                if (modalList.length === 0) return <div className="mx-4 my-3 border border-gray-200 bg-white rounded-xl flex flex-col items-center justify-center py-10 px-6 text-center"><Wind className="h-8 w-8 mb-3" style={{ color: '#e4e4e8' }} /><p className="text-sm font-medium text-gray-900">{t('Ni lokacij s kupljeno paro', 'No locations with purchased steam')}</p><p className="text-sm text-gray-500 mt-1 max-w-[280px] leading-relaxed">{t('Dodajte lokacijo, ki kupuje paro.', 'Add a location that purchases steam.')}</p></div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 z-10 bg-white border-b border-gray-200 px-6 py-5 flex items-center justify-between rounded-t-2xl">
              <div>
                <h2 className="text-base font-semibold text-gray-900">{entriesMap[activeLocation.id] ? t('Uredi vnos', 'Edit entry') : t('Dodaj porabo pare', 'Add steam consumption')}</h2>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">{t('Letna poraba pare', 'Annual steam consumption')} <span className="text-red-400">*</span></label>
                <div className="flex gap-2">
