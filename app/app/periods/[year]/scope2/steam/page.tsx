...
export default function Scope2SteamPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const [{ data: pd }, { data: locs }, { data: ents }] = await Promise.all([
        ...
        supabase.from('locations').select('id, name, address').eq('organization_id', org.id).eq('is_active', true).eq('uses_steam', true).order('name'),
        supabase.from('scope2_steam').select('*').eq('organization_id', org.id),
      ])
      setPeriod(pd)
      setLocations(locs ?? [])
  ...
  async function handleDelete() {
    ...
    const supabase = createClient()
    await supabase.from('scope2_steam').delete().eq('id', entry.id)
  ...
  return (
    <div className="p-4 lg:p-8">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-semibold text-[#0f0f10]">{t('Para – lokacije', 'Purchased Steam – locations')}</h1>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <div className="bg-white border border-[#ececec] rounded-xl p-4 flex items-center gap-3">
          <div className="w-9 h-9 bg-[#efefef] rounded-lg flex items-center justify-center shrink-0">
            <Leaf className="h-4 w-4 text-[#0f0f10]" />
          </div>
          <div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-[#ececec] rounded-xl py-14 text-center">
          ...
          <p className="text-sm text-[#767676] mb-1">{t('Ni lokacij z nakupljeno paro.', 'No locations with purchased steam.')}</p>
          <p className="text-xs text-[#767676]">{t('Označite lokacijo kot "uporablja paro" v razdelku Lokacije.', 'Mark a location as "uses steam" in the Locations section.')}</p>
        </div>
      ) : (
              ...
              {paginated.map((loc, i) => {
                ...
                return (
                  <tr key={loc.id} className={`hover:bg-[#f9f9f9] transition-colors ${i !== 0 ? 'border-t border-[#ececec]' : ''}`}>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-[#efefef] rounded-lg flex items-center justify-center shrink-0">
                          <Building2 className="h-4 w-4 text-[#0f0f10]" />
                        </div>
                        <div>
                    ...
                    <td className="px-5 py-4 text-sm text-[#767676]">{entry ? STEAM_FACTORS[entry.country_code]?.label ?? entry.country_code : <span className="text-gray-300">—</span>}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 z-10 bg-white border-b border-[#ececec] px-6 py-5 flex items-center justify-between rounded-t-2xl">
              <div>
                <h2 className="text-lg font-bold text-[#031f18]">{entriesMap[activeLocation.id] ? t('Uredi vnos', 'Edit entry') : t('Dodaj porabo pare', 'Add steam consumption')}</h2>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <select value={form.country_code} onChange={e => f('country_code', e.target.value)} className={SELECT}>
                  {COUNTRY_KEYS.map(k => <option key={k} value={k}>{STEAM_FACTORS[k].label}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-[#031f18] mb-1.5">{t('Letna poraba pare', 'Annual steam consumption')} <span className="text-red-400">*</span></label>
