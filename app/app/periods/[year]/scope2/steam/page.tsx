...
const COUNTRY_KEYS = Object.keys(STEAM_FACTORS)
  ...
  return (
    <div className="p-4 lg:p-8">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">{t('Para – lokacije', 'Purchased Steam – locations')}</h1>
        <p className="text-sm text-gray-400 mt-0.5">{t('Posredne emisije iz nakupljene pare', 'Indirect emissions from purchased steam')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-xl py-14 text-center">
          ...
          <p className="text-sm text-gray-500 mb-1">{t('Ni lokacij z nakupljeno paro.', 'No locations with purchased steam.')}</p>
          <p className="text-xs text-gray-400">{t('Označite lokacijo kot "uporablja paro" v razdelku Lokacije.', 'Mark a location as "uses steam" in the Locations section.')}</p>
        </div>
      ) : (
              ...
              {paginated.map((loc, i) => {
                ...
                return (
                  <tr key={loc.id} className={`hover:bg-gray-50 transition-colors ${i !== 0 ? 'border-t border-gray-100' : ''}`}>
                    ...
                    <td className="px-5 py-4 text-sm text-gray-600">{entry ? STEAM_FACTORS[entry.country_code]?.label ?? entry.country_code : <span className="text-gray-300">—</span>}</td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 z-10 bg-white border-b border-gray-100 px-6 py-5 flex items-center justify-between rounded-t-2xl">
              <div>
                <h2 className="text-lg font-bold text-gray-900">{entriesMap[activeLocation.id] ? t('Uredi vnos', 'Edit entry') : t('Dodaj porabo pare', 'Add steam consumption')}</h2>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('Letna poraba pare', 'Annual steam consumption')} <span className="text-red-400">*</span></label>
