...
export default function Scope2HeatPage() {
  ...
  async function saveSelection() {
    ...
    if (toAdd.length) await sup.from('period_locations').insert(toAdd.map(location_id => ({ reporting_period_id: period.id, location_id, organization_id: orgId, scope_type: 'heat' })))
  ...
  async function load(silent = false) {
    ...
    try {
      ...
      const [{ data: pd }, { data: locs }, { data: plRows }, { data: ents }] = await Promise.all([
        ...
        supabase.from('period_locations').select('id, location_id, reporting_period_id').eq('organization_id', org.id).eq('scope_type', 'heat'),
  ...
  async function handleSave() {
    ...
    try {
      ...
      load(true); refreshCounters(year); toast.success(t('Shranjeno', 'Saved')); setShowModal(false)
    ...
    <div className="flex flex-col h-full">
      <div className="flex flex-wrap items-center justify-between gap-2 px-4 sm:px-6 border-b border-gray-200 min-h-[57px] py-3 sm:h-[57px] sm:py-0 shrink-0">
        <div className="flex items-center gap-2 min-w-0">
          <h1 className="text-base font-semibold text-gray-900 shrink-0">{t('Toplota', 'Heat')}</h1>
          <p className="text-sm text-gray-500 truncate">{t('Vnesite letno porabo daljinskega ogrevanja znotraj poročevalskega obdobja po lokacijah.', 'Enter the annual district heat consumption for the reporting period by location.')}</p>
        </div>
              ...
              {paginated.map((loc, i) => {
                ...
                return (
                  <tr key={loc.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-sm text-gray-700">{entry ? HEAT_FACTORS[entry.country_code]?.label ?? entry.country_code : <span className="text-gray-300">—</span>}</td>
                    ...
                    <td className="px-5 py-3.5">{entry ? <span className="text-sm font-medium text-gray-900">{(entry.co2e_kg / 1000).toFixed(2).replace('.', ',')} tCO₂e</span> : <span className="text-gray-300 text-sm">—</span>}</td>
                    <td className="px-5 py-3.5">
                      {entry ? (
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[80vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto px-2 py-3 space-y-1.5">
              {(() => {
                ...
                if (modalList.length === 0) return <div className="mx-4 my-3 border border-gray-200 bg-white rounded-xl flex flex-col items-center justify-center py-10 px-6 text-center"><Flame className="h-8 w-8 mb-3" style={{ color: '#e4e4e8' }} /><p className="text-sm font-medium text-gray-900">{t('Ni lokacij z daljinsko toploto', 'No locations with district heat')}</p><p className="text-sm text-gray-500 mt-1 max-w-[280px] leading-relaxed">{t('Dodajte lokacijo, ki uporablja daljinsko toploto.', 'Add a location that uses district heat.')}</p></div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 z-10 bg-white border-b border-gray-200 px-6 py-5 flex items-center justify-between rounded-t-2xl">
              <div>
                <h2 className="text-base font-semibold text-gray-900">{entriesMap[activeLocation.id] ? t('Uredi vnos', 'Edit entry') : t('Dodaj porabo toplote', 'Add heat consumption')}</h2>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">{t('Država / vir omrežja', 'Country / grid source')}</label>
              ...
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">{t('Letna poraba toplote', 'Annual heat consumption')} <span className="text-red-400">*</span></label>
                <div className="flex gap-2">
            ...
            <div className="sticky bottom-0 bg-white border-t border-gray-200 px-6 py-4 flex gap-3 rounded-b-2xl">
              ...
              <button onClick={handleSave} disabled={saving || !form.kwh || parseQty(form.kwh) < 0}
                ...
                {entriesMap[activeLocation.id] ? t('Shrani', 'Save') : t('Dodaj vnos', 'Add entry')}
              </button>
            </div>
          </div>
        </div>
