...
import { Flame, Plus, X, Leaf, Check, Building2 } from 'lucide-react'
...
export default function Scope2HeatPage() {
  const { t } = useLocale()
  const params = useParams()
  const year = Number(params.year)
  const HEAT_FACTORS = getHeatFactors(year)
  ...
  async function load() {
    ...
    try {
      ...
      const [{ data: pd }, { data: locs }, { data: ents }] = await Promise.all([
        ...
        supabase.from('locations').select('id, name, address').eq('organization_id', org.id).eq('is_active', true).eq('uses_heat', true).order('name'),
        supabase.from('scope2_heat').select('*').eq('organization_id', org.id),
      ])
      setPeriod(pd)
      setLocations(locs ?? [])
  ...
  function co2ePreview(): number | null {
    ...
    const cf = HEAT_FACTORS[form.country_code]
  ...
  async function handleSave() {
    ...
    try {
      ...
      const { error: dbErr } = existing
        ? await supabase.from('scope2_heat').update(payload).eq('id', existing.id)
        : await supabase.from('scope2_heat').insert(payload)
  ...
  async function handleDelete() {
    ...
    const supabase = createClient()
    await supabase.from('scope2_heat').delete().eq('id', entry.id)
  ...
  return (
    <div className="p-4 lg:p-8">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-[#031f18]">{t('Toplota – lokacije', 'Purchased Heat – locations')}</h1>
        <p className="text-sm text-[#767676] mt-0.5">{t('Posredne emisije iz nakupljene daljinske toplote', 'Indirect emissions from purchased district heat')}</p>
      </div>
          ...
          <table className="w-full">
            <thead className="bg-[#fafafc]">
              <tr className="border-b border-[#ececec] bg-[#f9f9f9]/50">
                ...
                <th className="text-left text-xs font-semibold text-[#767676] uppercase tracking-wider px-5 py-3">{t('Država', 'Country')}</th>
              ...
              {paginated.map((loc, i) => {
                ...
                return (
                  <tr key={loc.id} className={`hover:bg-[#f9f9f9] transition-colors ${i !== 0 ? 'border-t border-[#ececec]' : ''}`}>
                    ...
                    <td className="px-5 py-4 text-sm text-[#767676]">{entry ? HEAT_FACTORS[entry.country_code]?.label ?? entry.country_code : <span className="text-gray-300">—</span>}</td>
                    ...
                    <td className="px-5 py-4">{entry ? <span className="text-sm font-semibold text-[#0f0f10]">{(entry.co2e_kg / 1000).toFixed(2).replace('.', ',')} tCO₂e</span> : <span className="text-gray-300 text-sm">—</span>}</td>
                    <td className="px-5 py-4">
                      {entry ? (
                        <span className="inline-flex items-center gap-1.5 text-xs font-medium px-2 py-0.5 rounded-full bg-[#f5f5f5] text-[#0f0f10]"><span className="w-1.5 h-1.5 rounded-full bg-[#f5f5f5]0" />{t('Podatki vneseni', 'Data entered')}</span>
                      ) : (
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1 justify-end">
                        {entry ? (
                          <button onClick={() => openEdit(loc)} className="inline-flex items-center gap-1 text-xs font-semibold text-[#767676] hover:text-[#031f18] px-3 py-1.5 bg-white border border-[#ececec] hover:bg-[#f9f9f9] rounded-lg transition-colors">
                          ...
                          <button onClick={() => openAdd(loc)} className="inline-flex items-center gap-1 text-xs font-semibold text-[#0f0f10] hover:text-[#0f0f10] px-3 py-1.5 bg-[#efefef] hover:bg-[#d4eddf] rounded-lg transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 z-10 bg-white border-b border-[#ececec] px-6 py-5 flex items-center justify-between rounded-t-2xl">
              <div>
                <h2 className="text-lg font-bold text-[#031f18]">{entriesMap[activeLocation.id] ? t('Uredi vnos', 'Edit entry') : t('Dodaj porabo toplote', 'Add heat consumption')}</h2>
                <p className="text-xs text-[#767676] mt-0.5">{activeLocation.name} · Scope 2 · {year}</p>
              </div>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-[#031f18] mb-1.5">{t('Država / vir omrežja', 'Country / grid source')}</label>
                <select value={form.country_code} onChange={e => f('country_code', e.target.value)} className={SELECT}>
                  {COUNTRY_KEYS.map(k => <option key={k} value={k}>{HEAT_FACTORS[k].label}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-[#031f18] mb-1.5">{t('Letna poraba toplote', 'Annual heat consumption')} <span className="text-red-400">*</span></label>
                <div className="flex gap-2">
                  ...
                  <div className="w-12 px-2 py-2 text-sm bg-[#f9f9f9] border border-[#ececec] rounded-lg text-[#767676] flex items-center justify-center shrink-0 font-medium">kWh</div>
                </div>
              </div>
              {preview !== null && (
