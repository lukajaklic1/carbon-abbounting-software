  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between px-6 border-b border-gray-200 h-[57px] shrink-0">
        <div>
          <h1 className="text-base font-semibold text-gray-900">{t('Toplota – lokacije', 'Purchased Heat – locations')}</h1>
      </div>
      ...
      {loading ? (
        ...
        <EmptyState icon={Flame} title={t('Ni lokacij z daljinsko toploto', 'No locations with district heat')} subtitle={t('Dodajte lokacijo z daljinsko toploto.', 'Add a location with district heat enabled.')} />
      ) : (
        <>
              ...
              {paginated.map((loc, i) => {
                ...
                return (
                  <tr key={loc.id} className={`hover:bg-gray-50 transition-colors ${i !== 0 ? 'border-t border-gray-200' : ''}`}>
                    ...
                    <td className="px-5 py-3.5">{entry ? <span className="text-sm font-medium text-gray-700">{(entry.co2e_kg / 1000).toFixed(2).replace('.', ',')} tCO₂e</span> : <span className="text-gray-300 text-sm">—</span>}</td>
                    <td className="px-5 py-3.5">
                      {entry ? (
