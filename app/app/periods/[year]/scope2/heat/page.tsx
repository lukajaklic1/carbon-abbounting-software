...
import { HEAT_FACTORS, calcCo2eKg } from '@/lib/emission-factors'
...
const COUNTRY_KEYS = Object.keys(HEAT_FACTORS)
...
export default function Scope2HeatPage() {
  ...
  function openEdit(loc: any) {
    ...
    setForm({ kwh: Number(e.quantity ?? 0).toLocaleString('sl-SI', { maximumFractionDigits: 6 }), country_code: e.country_code ?? 'SI', data_source: e.data_source ?? '', notes: e.notes ?? '' })
    setActiveLocation(loc); setError(''); setShowModal(true)
  }
  ...
  return (
    <div className="p-4 lg:p-8">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">{t('Toplota – lokacije', 'Purchased Heat – locations')}</h1>
        <p className="text-sm text-gray-400 mt-0.5">{t('Posredne emisije iz nakupljene daljinske toplote', 'Indirect emissions from purchased district heat')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-xl py-14 text-center">
          ...
          <p className="text-sm text-gray-500 mb-1">{t('Ni lokacij z nakupljeno toploto.', 'No locations with purchased heat.')}</p>
          <p className="text-xs text-gray-400">{t('Označite lokacijo kot "uporablja daljinsko toploto" v razdelku Lokacije.', 'Mark a location as "uses district heat" in the Locations section.')}</p>
        </div>
      ) : (
              ...
              {paginated.map((loc, i) => {
                ...
                return (
                  <tr key={loc.id} className={`hover:bg-gray-50 transition-colors ${i !== 0 ? 'border-t border-gray-100' : ''}`}>
                    ...
                    <td className="px-5 py-4 text-sm text-gray-600">{entry ? HEAT_FACTORS[entry.country_code]?.label ?? entry.country_code : <span className="text-gray-300">—</span>}</td>
                      ...
                      <div className="flex items-center gap-1 justify-end">
                        {entry ? (
                          ...
                          <button onClick={() => openAdd(loc)} className="inline-flex items-center gap-1 text-xs font-semibold text-blue-600 hover:text-blue-700 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 rounded-lg transition-colors">
                            <Plus className="h-3 w-3" />{t('Dodaj', 'Add')}
                          </button>
                        )}
                      </div>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 z-10 bg-white border-b border-gray-100 px-6 py-5 flex items-center justify-between rounded-t-2xl">
              <div>
                <h2 className="text-lg font-bold text-gray-900">{entriesMap[activeLocation.id] ? t('Uredi vnos', 'Edit entry') : t('Dodaj porabo toplote', 'Add heat consumption')}</h2>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('Država / vir omrežja', 'Country / grid source')}</label>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">{t('Letna poraba toplote', 'Annual heat consumption')} <span className="text-red-400">*</span></label>
              ...
              <div>
                ...
                <input value={form.data_source} onChange={e => f('data_source', e.target.value)} placeholder={t('npr. račun za toploto', 'e.g. heat bill')} className={INPUT} />
              </div>
              <div>
