  ...
  return (
    <div className="p-4 lg:p-8">
      <div className="mb-6">
        ...
        <p className="text-sm text-[#455451] mt-0.5">{t('Posredne emisije iz nakupljene daljinske toplote', 'Indirect emissions from purchased district heat')}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-[#e2e2e4] rounded-xl py-14 text-center">
          ...
          <p className="text-sm text-[#455451] mb-1">{t('Ni lokacij z nakupljeno toploto.', 'No locations with purchased heat.')}</p>
          <p className="text-xs text-[#455451]">{t('Označite lokacijo kot "uporablja daljinsko toploto" v razdelku Lokacije.', 'Mark a location as "uses district heat" in the Locations section.')}</p>
        </div>
      ) : (
          ...
          <table className="w-full">
            <thead className="bg-[#fafafc]">
              <tr className="border-b border-[#e2e2e4] bg-[#f9f9f9]/50">
                ...
                <th className="text-left text-xs font-semibold text-[#455451] uppercase tracking-wider px-5 py-3">{t('Država', 'Country')}</th>
              ...
              {paginated.map((loc, i) => {
                ...
                return (
                  <tr key={loc.id} className={`hover:bg-[#f9f9f9] transition-colors ${i !== 0 ? 'border-t border-[#e2e2e4]' : ''}`}>
                    ...
                    <td className="px-5 py-4 text-sm text-[#455451]">{entry ? HEAT_FACTORS[entry.country_code]?.label ?? entry.country_code : <span className="text-gray-300">—</span>}</td>
