  ...
  return (
    <div className="p-4 lg:p-8">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Oprema</h1>
      </div>
      {loading ? (
        <div className="bg-white border border-gray-200 rounded-xl p-12 text-center text-sm text-gray-400">Nalaganje...</div>
      ) : rows.length === 0 ? (
        <div className="bg-white border border-gray-200 rounded-xl p-12 text-center text-sm text-gray-300">Ni opreme.</div>
      ) : (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                ...
                <th className="text-left text-xs font-semibold text-gray-400 uppercase tracking-wider px-5 py-3">Kategorija</th>
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={r.id} className={cn('hover:bg-gray-50', i !== 0 && 'border-t border-gray-100')}>
                  ...
                  <td className="px-5 py-3.5 text-sm text-gray-600">{r.category ?? r.fuel_type ?? '—'}</td>
                  <td className="px-5 py-3.5">
