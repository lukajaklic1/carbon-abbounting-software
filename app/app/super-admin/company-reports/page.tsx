...
function ScopeSection({ label, sub, data }: { label: string; sub: string; data: { total: number; sources: SourceRow[] } }) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
      <button onClick={() => setOpen(v => !v)}
        ...
        <div className="flex items-center gap-4">
          <div className="text-right">
            ...
            <p className="text-xs text-gray-400">Bruto tCO₂e</p>
          </div>
          ...
          {data.sources.map(src => (
            <div key={src.name} className="rounded-xl bg-gray-50 overflow-hidden">
              <button onClick={() => toggle(src.name)}
                ...
                <ChevronDown className={cn('h-4 w-4 text-gray-400 shrink-0 transition-transform', openSources.has(src.name) && 'rotate-180')} />
              </button>
              {openSources.has(src.name) && (
                <div className="border-t border-gray-200">
                  {[
                    ...
                    <div key={f.label} className="flex items-center justify-between px-4 py-2.5 border-b border-gray-100 last:border-0 bg-white">
      ...
      {open && data.sources.length === 0 && (
        <div className="border-t border-gray-100 px-6 py-8 text-center text-sm text-gray-400">Ni vnosov za to leto.</div>
      )}
    </div>
  )
}
  ...
  return (
    <div className="p-4 lg:p-8 max-w-3xl mx-auto">
      <div className="flex items-start justify-between mb-8 flex-wrap gap-3">
        <div>
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1">GHG · {selectedOrg.name} · {year}</p>
          <h1 className="text-2xl font-bold text-gray-900">Poročilo o metodologiji GHG</h1>
          <p className="text-sm text-gray-400 mt-1">Pregled metodologij in emisijskih faktorjev za izračun ogljičnega odtisa.</p>
        </div>
        <button
          onClick={async () => {
            if (!report) return
      ...
      {loading ? (
        <div className="bg-white border border-gray-200 rounded-2xl p-12 text-center text-sm text-gray-400">Nalaganje...</div>
      ) : (
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6">
            ...
            <div>
              {[
                ...
                <div key={r.label} className={cn('flex items-center justify-between py-3', i < 2 && 'border-b border-gray-100')}>
