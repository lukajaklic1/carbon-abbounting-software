...
export default function AdminScope3Page() {
  ...
  return (
    <div className="p-4 lg:p-8 max-w-5xl mx-auto">
      <div className="mb-6">
        <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-1">Super Admin</p>
        <h1 className="text-2xl font-bold text-gray-900">Scope 3 — Oddane datoteke</h1>
        <p className="text-sm text-gray-400 mt-0.5">Pregled oddanih podatkov strank in vnos izračunanih emisij.</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-gray-400">V pregledu</p>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-gray-400">Zaključeno</p>
        ...
        {(['all', 'in_review', 'done'] as const).map(f => (
          <button key={f} onClick={() => setFilterStatus(f)}
            className={cn('px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors',
              filterStatus === f ? 'bg-gray-900 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'
            )}>
      ...
      {loading ? (
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-12 text-center text-sm text-gray-400">Ni oddanih podatkov.</div>
      ) : (
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          {filtered.map((sub, i) => (
            <div key={sub.id} className={cn('px-5 py-4', i !== 0 && 'border-t border-gray-100')}>
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-gray-900">{CATEGORY_LABELS[sub.category_number]}</p>
                  <p className="text-xs text-gray-400 mt-0.5">{sub.org_name} · {sub.period_year}</p>
                </div>
                ...
                {sub.status === 'done' && (
                  <button onClick={() => reopen(sub)}
                    className="px-3 py-1.5 text-xs font-semibold bg-white border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-50 transition-colors shrink-0">
                    Znova odpri
                  </button>
                )}
              </div>
