    ...
    async function load() {
      try {
        ...
        setCounters({
          ...
          [`/app/periods/${y}/scope1/stationary`]:       { done: statCount ?? 0, total: plStatCount ?? 0 },
          [`/app/periods/${y}/scope1/mobile`]:           { done: mobCount ?? 0,  total: periodVehCount ?? 0 },
          [`/app/periods/${y}/scope1/equipment-fuel`]:   { done: efCount ?? 0,   total: peFuelCount ?? 0 },
          [`/app/periods/${y}/scope1/refrigerants`]:     { done: refCount ?? 0,  total: peRefCount ?? 0 },
          [`/app/periods/${y}/scope1/industrial-gases`]: { done: gasCount ?? 0,  total: peGasCount ?? 0 },
          [`/app/periods/${y}/scope2/electricity`]:      { done: elecCount ?? 0, total: plElecCount ?? 0 },
          [`/app/periods/${y}/scope2/heat`]:             { done: heatCount ?? 0, total: plHeatCount ?? 0 },
          [`/app/periods/${y}/scope2/steam`]:            { done: steamCount ?? 0, total: plSteamCount ?? 0 },
          [`/app/periods/${y}/scope2/cooling`]:          { done: coolCount ?? 0, total: plCoolCount ?? 0 },
        })
      } catch (e) {
