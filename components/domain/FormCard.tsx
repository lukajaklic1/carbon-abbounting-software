...
export function FormInput({ name, placeholder, defaultValue, required, maxLength }: {
  ...
  return (
    <input
      ...
      required={required}
      maxLength={maxLength}
      className="w-full px-3 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent placeholder:text-gray-300"
    />
  )
}
...
export function FormSelect({ name, defaultValue, children }: {
  ...
  return (
    <select
      name={name}
      defaultValue={defaultValue}
      className="w-full px-3 py-2.5 text-sm bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-transparent"
    >
      {children}
    </select>
  )
